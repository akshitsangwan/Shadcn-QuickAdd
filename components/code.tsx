...
export default function Code() {
  ...
  return (
    <Card className="w-full max-w-xl mx-auto rounded-xl shadow border border-border/60 bg-background">
      ...
      <CardContent>
        ...
        >
              ...
              <TabsList className="bg-transparent gap-2">
                {Object.keys(installCommands).map((pkg) => (
                  <TabsTrigger
                    key={pkg}
                    value={pkg}
                    className="px-5 py-1 rounded data-[state=active]:bg-primary data-[state=active]:text-primary-foreground"
                  >
                    {pkg}
                  </TabsTrigger>
                ))}
          ...
          {Object.entries(installCommands).map(([pkg, cmd]) => (
            ...
            >
              <ScrollArea>
                <div className="flex items-center justify-between bg-zinc-900 text-white font-mono text-sm px-4 py-3 rounded-lg border border-zinc-800">
